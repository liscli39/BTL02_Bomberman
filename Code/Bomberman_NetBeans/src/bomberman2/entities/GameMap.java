...
import bomberman2.GamePanelController;
...
//         Graphic.printCharMap();
    ...
    private void enemyMovement(){
        this.graphic.getBallooms().forEach(e->{
            e.dumpMove();
            Player pl =this.graphic.getPlayers().get(0);
            if(Collision.isCollision(pl, e)){
                if(this.isPvsP){
                    pl.setDeath(true);
                }else
                if(pl.getNumLife()>1){
    ...
    private void bombExplosion(){
        ...
        while(ib.hasNext()){
            ...
            if(!b.countDown() || isExplosive){
                ...
                if(GamePanelController.isSound){
                    Sound.playSound("src/bomberman2/sound/bombExp.wav");
                }
                // burn
